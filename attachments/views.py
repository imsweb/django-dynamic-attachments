...
class AttachView(ContextMixin, View):
    ...
    def validate_extension(self, path):
        ...
        if self.session.allowed_file_extensions:
            ...
            mime_types_overrides = getattr(
                settings, "ATTACHMENTS_MIME_TYPE_OVERRIDES", {}
            )
    ...
    def scan_clamd(self, path):
        ...
        if getattr(settings, "ATTACHMENTS_CLAMD", False):
            ...
            clamd_network_settings = getattr(
                settings, "ATTACHMENTS_CLAMD_NETWORK_SETTINGS", {}
            )
            ...
            if clamd_network_settings:
                ...
                cd = Centos7ClamdUnixSocket(
                    filename=getattr(
                        settings, "ATTACHMENTS_CLAMD_UNIX_SOCKET_LOCATION", None
                    )
                )
    ...
    def handle_virus_found(self, upload, ex):
        ...
        attachments_quarantine_path = getattr(
            settings, "ATTACHMENTS_QUARANTINE_PATH", None
        )
        if attachments_quarantine_path:
            quarantine_path = os.path.join(
                attachments_quarantine_path, os.path.basename(upload.file_path)
            )
        ...
        log_message = (
            f"{error_msg} - Uploaded by {user} at {time_of_upload}. {quarantine_msg}."
        )
        ...
        return JsonResponse(
            {"ok": False, "error": error_msg}, content_type=self.content_type
        )
    ...
    def handle_file_upload(self, request, *args, **kwargs):
        ...
        # These errors have helpful messages, so we return them
            ...
            return JsonResponse(
                {"ok": False, "error": force_str(ex)}, content_type=self.content_type
            )
        except VirusFoundException as ex:
            return self.handle_virus_found(upload, ex)
        except ValidationError as ex:
            return JsonResponse(
                {"ok": False, "error": force_str(ex.message)},
                content_type=self.content_type,
            )
        except Exception:
            logger.exception(
                f'Error attaching file to session {self.kwargs["session_id"]}'
            )
            # Since this is a catch all we need to return a generic error (in case a more sensitive error occurred)
            return JsonResponse(
                {
                    "ok": False,
                    "error": "An error occurred attaching file to the session.",
                },
                content_type=self.content_type,
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        self.content_type = content_types.get(
            request.POST.get("X-Requested-With"), "application/json"
        )
        ...
        if self.session.unpack_zip_files and self.file_is_zipped:
            ...
            with ZipFile(self.file, "r") as zf:
                files = [
                    SimpleUploadedFile(
                        ...
                        content=BytesIO(zf.read(fileinfo.filename)).getbuffer(),
                    )
            ...
            for self.file in files:
                ...
                if self.file_is_zipped:
                    errors.append(
                        f"{self.file.name} - Error: nested zip files are not supported."
                    )
                else:
                    json_content = json.loads(
                        self.handle_file_upload(request, *args, **kwargs).content
                    )
...
def delete_upload(request, session_id, upload_id):
    ...
    try:
        ...
        logger.exception(
            "Error deleting upload (pk=%s, file_name=%s) from session %s",
            upload_id,
            file_name,
            session_id,
        )
...
def download(request, attach_id, filename=None):
    ...
    if getattr(settings, "ATTACHMENTS_USE_XSENDFILE", False):
        ...
        response = StreamingHttpResponse(
            FileWrapper(storage.open(attachment.file_path)), content_type=content_type
        )
    try:
                ...
                return JsonResponse(
                    {
                        ...
                        "form_html": loader.render_to_string(
                            "attachments/form.html", {"form": property_form}
                        ),
                    }
                )
        except Exception as ex:
            logger.exception(
                "Error updating attachment (pk=%s, file_name=%s)",
                attach_id,
...
def edit_attachment_properties(request, attach_id):
    ...
    return render(
        request,
        "attachments/edit_properties.html",
        {
            "att": attachment,
...
def view_attachment_properties(request, attach_id):
    ...
    return render(
        request,
        "attachments/view_properties.html",
        {
            "att": attachment,
        },
    )
