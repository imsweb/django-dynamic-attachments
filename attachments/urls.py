...
urlpatterns = patterns('attachments.views',
    ...
    url(r'^update/attachment/(?P<content_type>[^/]+)/(?P<attach_id>[^/]+)/$', 'update_attachment', name='update-attachment'),
    url(r'^show/attachment/(?P<content_type>[^/]+)/(?P<attach_id>[^/]+)/$', 'show_attachment_properties', name='show-attachment-properties'),
)
